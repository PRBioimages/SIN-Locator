...
good_feature_idx = [];  notgood_feature_idx = [];
for i=1:length(ProbabilityF)
    PrF = ProbabilityF{i};
%     good_feature_idx = [good_feature_idx i];
    if strcmp(PrF, '<.0001')
        if length(good_feature_idx)<maxF
            good_feature_idx = [good_feature_idx; i];
        end
    else
        notgood_feature_idx = [notgood_feature_idx; [i str2double(PrF)]];
    end
end
if length(good_feature_idx)< minF && ~isempty(notgood_feature_idx)  % if too few selected features
    [~, ind] = sort(notgood_feature_idx(:,2));
    if size(notgood_feature_idx,1)>=minF-length(good_feature_idx)
        good_feature_idx = [good_feature_idx; notgood_feature_idx(ind(minF-length(good_feature_idx)),1)];
    else
        good_feature_idx = [good_feature_idx; notgood_feature_idx(ind,1)];
    end
end

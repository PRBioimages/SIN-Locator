...
function [X,Labels,Values,confusion]=ECOCTest(TestData,Classifiers,Parameters,labels)
...
if size(TestData,1)~=0
    ...
    if size(ECOC,1)==0
        ...
        [X,result,confusion,Labels,Values]=Decoding(TestData,classes,ECOC,Parameters.base,Classifiers,Parameters.decoding,Parameters.base_test,Parameters.base_test_params,Parameters.custom_decoding,Parameters.custom_decoding_params);
    end
else
    Labels=[];
    Values=[];
